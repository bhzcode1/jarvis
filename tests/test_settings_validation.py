...
def test_validate_runtime_settings_detects_bad_values() -> None:
    ...
    assert any("COMMAND_WAIT_FOR_SPEECH_SECONDS must be greater than 0" in item for item in warnings)
    assert any("COMMAND_SILENCE_TIMEOUT_SECONDS must be greater than 0" in item for item in warnings)
    assert any("COMMAND_MAX_DURATION_SECONDS must be greater than 0" in item for item in warnings)
